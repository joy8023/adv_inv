...
def train(classifier, inversion, inversion_def, log_interval, device, data_loader, optimizer, epoch, num_step = 10):
    classifier.eval()
    inversion.train()

    epsilon = 10e-5

    for batch_idx, (data_, out) in enumerate(data_loader):
        data, out = data_.to(device), out.to(device)
        optimizer.zero_grad()
        ...
        for i in range(num_step):
            ...
            reconstruction = inversion(F.softmax(logit, dim=1))
            loss_ =F.mse_loss(reconstruction, data)
            loss_.backward()
        ...
        reconstruction = inversion(perturbation)
        loss = F.mse_loss(reconstruction, data_)
        loss.backward()
        optimizer.step()
...
def main():
    ...
    inversion_def = nn.DataParallel(Inversion(nc=args.nc, ngf=args.ngf, nz=args.nz, truncation=args.truncation, c=args.c)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train(classifier, inversion, inversion_def, args.log_interval, device, train_loader, optimizer, epoch)
        ...
        if recon_loss < best_recon_loss:
            ...
            state = {
                'epoch': epoch,
                'model': inversion_def.state_dict(),
                'optimizer': optimizer.state_dict(),
                'best_recon_loss': best_recon_loss
            }
            torch.save(state, 'model/inversion_rtdef.pth')
            #torch.save(inversion.state_dict(), 'model/inv_model_def_dict.pth')
            shutil.copyfile('out/recon_test1_def{}.png'.format(epoch), 'out/best_test1_rtdef.png')
