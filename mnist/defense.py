...
def perturb(prediction, epsilon, grad, logit_original, logit = True):
	...
	#logit_new = logit_original + (logit_new - logit_original)/logit_diff

	original_label = torch.max(logit_original, 1)[1].cpu().numpy()
	new_label = torch.max(logit_new, 1)[1].cpu().numpy()

	orig_label_onehot = F.one_hot(torch.tensor(original_label), 10)
	...
	#output_diff = F.l1_loss(output, F.softmax(prediction, dim=1))
	print('************output_diff', logit_diff.item())
